...
var option_naive_prot = ["https", "quic", "anytls"];						//fancyss-full
...
function get_tuic_relay_server(value) {
	...
	if (typeof tuicJson == "string") {
		try {
			tuicJson = JSON.parse(tuicJson);
		} catch (e) {
			return "";
		}
	}
	...
	return String(tuicJson.relay.server);
}
function get_tuic_relay_host(value) {
	var relayServer = get_tuic_relay_server(value);
	if (!relayServer) {
		return "";
	}
...
function has_vless_seed_encryption(protocol, encryption) {
	...
	return protocol == "vless" && encryption !== "" && encryption !== "none";
}
function should_show_xray_flow(network, security, protocol, encryption) {
	network = (network || "tcp").toLowerCase();
	security = (security || "none").toLowerCase();
	if (has_vless_seed_encryption(protocol, encryption)) {
		return true;
	}
	return network == "tcp" && (security == "tls" || security == "reality");
}
function get_node_table_xray_protocol() {
	if (edit_id && confs && confs[edit_id] && confs[edit_id]["type"] == "4") {
		return confs[edit_id]["xray_prot"] || "vless";
	}
	return "vless";
}
function build_node_derived_info(obj) {
	...
	if (obj["type"] == "7" && obj["tuic_json"]) {
		try {
			...
			var tuicServer = get_tuic_relay_host(tuicJson);
			if (tuicServer) {
				obj["server"] = tuicServer;
			}
		} catch (e) {}
	}
	var jsonField = "";
...
function clear_front_status_http_watchdog() {
	if (statusFrontHttpWatchdog) {
		clearTimeout(statusFrontHttpWatchdog);
		statusFrontHttpWatchdog = null;
	}
}
function clear_front_status_http_abort() {
...
function refresh_options() {
	...
	for (var i = 0; i < ss_nodes.length; i++) {
		...
		if (c.type == "0"){
			...
			option0.append($("<option>", {
				value: field,
				text: c["xray_use_json"] == "1" ? "【json】" + group_tag + c.name : "【" + xrayLabel + "】" + group_tag + c.name
			}));
		}
		else if(c.type == "5"){
			//trojan
			option0.append($("<option>", {
				...
				option0.append($("<option>", {																					//fancyss-full
					...
					text: (c["naive_prot"] == "anytls" ? "【AnyTLS】" : "【Naïve】") + group_tag + c.name																		//fancyss-full
				}));																											//fancyss-full
			}																													//fancyss-full
...
function save() {
	...
	if (node_type == "4" ){
		...
		if (E("ss_basic_xray_use_json").checked == true){
			...
			var xrayProt = dbus["ssconf_basic_xray_prot_" + node_sel] || "vless";
			var xrayFlowVisible = should_show_xray_flow(E("ss_basic_xray_network").value, E("ss_basic_xray_network_security").value, xrayProt, E("ss_basic_xray_encryption").value);
			dbus["ssconf_basic_xray_flow_" + node_sel] = xrayFlowVisible ? E("ss_basic_xray_flow").value : "";
	...
	if (node_type == "7" ){
		...
		if(isJSON(E('ss_basic_tuic_json').value)){
			if(get_tuic_relay_server(E('ss_basic_tuic_json').value)){
...
function getNodeCtx() {
	...
	ctx.x_flow_on = should_show_xray_flow(x_net, getFieldValue("ss_node_table_xray_network_security"), get_node_table_xray_protocol(), getFieldValue("ss_node_table_xray_encryption"));
...
function verifyFields(r) {
	...
	var ctx = {
		...
		x_ai_off: x_ai_off,
		x_ai_on: x_ai_on,
		x_flow_on: should_show_xray_flow(x_net, E("ss_basic_xray_network_security").value, dbus["ssconf_basic_xray_prot_" + node_sel] || "vless", E("ss_basic_xray_encryption").value),
	...
	if(E("ss_adv_sub").checked == false){
		...
		$("#ss_basic_sub_keep_info_node").parent().parent().hide();
		...
		$("#ss_basic_sub_keep_info_node").parent().parent().show();
...
function add_ss_node_conf(flag) {
	...
	if (flag == 'shadowsocks') {
		...
		if (E("ss_node_table_xray_use_json").checked == true){
			...
			if (!should_show_xray_flow(E("ss_node_table_xray_network").value, E("ss_node_table_xray_network_security").value, "vless", E("ss_node_table_xray_encryption").value)) {
				ns[p + "_xray_flow_" + node_id] = "";
			}
	...
	//fancyss_tuic_1
		...
		if ($("#ss_node_table_tuic_json").val()){
			if(isJSON(E('ss_node_table_tuic_json').value)){
				if(get_tuic_relay_server(E('ss_node_table_tuic_json').value)){
...
function edit_ss_node_conf(flag) {
	...
	if (flag == 'shadowsocks') {
		...
		if (E("ss_node_table_xray_use_json").checked == true){
			...
			if (!should_show_xray_flow(E("ss_node_table_xray_network").value, E("ss_node_table_xray_network_security").value, get_node_table_xray_protocol(), E("ss_node_table_xray_encryption").value)) {
				ns[p + "_xray_flow_" + edit_id] = "";
			}
			ns[p + "_xray_prot_" + edit_id] = get_node_table_xray_protocol();
		}
...
function generate_node_info() {
	...
	for (var j = 0; j < ss_nodes.length; j++) {
		...
		if(db_ss[p + "_type_" + idx] ==  "7"){
			...
			var server_addr = get_tuic_relay_host(json);
	...
	//for (var field in confs) {
		...
		switch(c["type"]) {
			...
			case '5' :
				...
				break;
				case '6' :														//fancyss-full
					html += (c["naive_prot"] == "anytls" ? "AnyTLS" : "Naïve");												//fancyss-full
					break;														//fancyss-full
			case '7' :														//fancyss-full
...
function build_naive_standard_link(c) {
	if (!c || !c["naive_server"] || !c["naive_pass"]) {
		return 7;
	}
	var prot = c["naive_prot"] || "https";
	if (prot != "https" && prot != "quic" && prot != "anytls") {
		return 7;
	}
	var password = "";
	try {
	...
	var uri = "";
	if (prot == "anytls") {
		uri = "anytls://" + encodeURIComponent(password) + "@" + format_share_remote_host(c["naive_server"]);
		if (c["naive_port"]) {
			uri += ":" + c["naive_port"];
		}
		if (c["naive_user"]) {
			uri += "/?sni=" + encodeURIComponent(c["naive_user"]);
		}
	} else {
		if (!c["naive_user"]) {
			return 7;
		}
		uri = "naive+" + prot + "://" + encodeURIComponent(c["naive_user"]) + ":" + encodeURIComponent(password) + "@" + format_share_remote_host(c["naive_server"]);
		if (c["naive_port"]) {
			uri += ":" + c["naive_port"];
		}
	}
...
function showQRcode(data) {
	...
	if(data == 1){
		...
		$("#qrcode").html('<span style="font-size:15px;color:#000;line-height:1.7;">当前Naïve/AnyTLS节点缺少生成标准二维码所需的基础信息。<br />请检查协议、服务器与密码（Naïve还需账户）是否完整。</span>')
	}
	else
	{
...
function sync_acl_checkbox_state_by_mode(udpId, quicId, mode) {
	...
	if (!acl_current_node_supports_udp()) {
		...
		sync_acl_checkbox_ui(udpId, "当前Naïve/AnyTLS节点不支持UDP代理，已忽略此开关", true);
		sync_acl_checkbox_ui(quicId, "当前Naïve/AnyTLS节点不支持UDP代理，默认屏蔽QUIC流量", true);
		return;
	}
...
function save_online_nodes(action) {
	...
	if (action == "4"){
		...
		if (action == "2"||action == "3"){
			...
			dbus_post["ss_basic_sub_keep_info_node"] = E("ss_basic_sub_keep_info_node").checked ? "1":"0";
										...
										<td bgcolor="#4D595D" colspan="3" valign="top">
											...
											<div class="SimpleNote" id="head_illustrate">
												<ul id="fixed_msg" style="padding:0;margin:0;line-height:1.8;">
													<li id="msg_0" style="list-style: none;height:23px">
														...
														、<a href="https://github.com/anytls/anytls-go" target="_blank"><em><u>AnyTLS</u></em></a>
															...
															<table id="table_add_nodes" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
																<script type="text/javascript">
																	$('#table_add_nodes').forms([
																		...
																		// naive
																		{ title: 'NaïveProxy/AnyTLS 协议', data:{show:'naive_on'}, id:'ss_node_table_naive_prot', type:'select', func:'v', options:option_naive_prot, maxlen:'300', style:'width:412px', value: "https"},		//fancyss-full
																		...
																		{ title: 'NaïveProxy 账户 / AnyTLS SNI', data:{show:'naive_on'}, id:'ss_node_table_naive_user', type:'text', maxlen:'300', style:'width:400px'},															//fancyss-full
												...
												<table id="table_basic" width="100%" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
													<script type="text/javascript">
														$('#table_basic').forms([
															...
															// naive
															{ title: 'NaïveProxy/AnyTLS 协议', id:'ss_basic_naive_prot', data:{show:'naive_on'}, type:'select', func:'v', options:option_naive_prot, maxlen:'300', value: "https"},								//fancyss-full
															...
															{ title: 'NaïveProxy 账户 / AnyTLS SNI', id:'ss_basic_naive_user', data:{show:'naive_on'}, type:'text', maxlen:'300'},																						//fancyss-full
											...
											<div id="tablet_7" style="display: none;">
												...
												<table id="table_subscribe" style="margin:8px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
													<script type="text/javascript">
														...
														$('#table_subscribe').forms([
															...
															{ title: '自定义UserAgent', id:'ss_basic_online_ua', type:'select', style:'width:auto', hint:'112', options:[["0", "fancyss默认（≥3.3.9）"], ["1", "curl/wget（≤3.3.8）"], ["2", "V2rayN"], ["3", "V2rayNG"], ["4", "Shadowrocket"], ["5", "ClashMeta/ClashVerge"]], value:'0'},
															{ title: '订阅计划任务', multi: [
												...
												<table id="table_link" style="margin:8px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
													<script type="text/javascript">
														var ph1 = "填入以ss://、ssr://、vmess://、vless://、trojan://、hysteria2://、hy2://、tuic://、naive+https://、naive+quic://、anytls://开头的链接，多个链接请分行填写";
														$('#table_subscribe').forms([
